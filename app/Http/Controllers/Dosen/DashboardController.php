...
class DashboardController extends Controller
{
    public function index (){
		...
		// jumlah user
        $total_users = DB::table('users')->where('level','mahasiswa')->count();
        ...
        for ($i=1; $i <= $bulan ; $i++){
            ...
            $totalusers = DB::table('users')->whereYear('created_at', $tahun)->whereMonth('created_at', $i)->where('level','mahasiswa')->count();
